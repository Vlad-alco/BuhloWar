...
String testAwaitingType = "";     // "head", "body_nc", "body_no"
    ...
    float bodyVolDone = 0.0f;        // Накопленный объём тела (мл)
    ...
    struct CalibWizard {
        ...
        int step = 0;            // 0=idle, 1=dry_run, 2=capacity, 3=input_volume, 4=result
...
enum class UiCommand {
    ...
    CALIB_START_CAPACITY,      // Начать измерение capacity (60 сек, цикл 1с/1с)
...
struct CommandMessage {
    ...
    CommandMessage() : command(UiCommand::NONE), param(0) {}
    CommandMessage(UiCommand cmd, int p) : command(cmd), param(p) {}
};
